...
public class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    public override func didMove(to view: SKView) {
        ...
        // background
        let bg = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "clouds_back.jpg")))
        bg.setScale(2.0)
        bg.zPosition = -10
        ...
        //player
        player = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "clipart1199528.png")), color: .clear, size: CGSize(width: size.width * 0.05, height: size.width * 0.05))
        player.position = CGPoint(x: frame.midX, y: frame.midY)
        player.addCircle(radius: player.size.width * (0.6 + socialDistancingLength), edgeColor: .green, filled: true)
        addChild(player)
        ...
        for person in people {
            ...
            infectedEnemy.texture = SKTexture(image: #imageLiteral(resourceName: "ragazzino-04.png"))
    ...
    func createPerson() {
        let person = SKSpriteNode(texture: SKTexture(image: #imageLiteral(resourceName: "wind.png")), color: .clear, size: CGSize(width: size.width * 0.05, height: size.width * 0.05))
        ...
        person.addCircle(radius: player.size.width * (0.6 + socialDistancingLength), edgeColor: .lightGray, filled: false)
        ...
        person.physicsBody = SKPhysicsBody(circleOfRadius: player.size.width * (0.6 + socialDistancingLength))
    ...
    public override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        player.run(SKAction.move(to: newPlayerPos, duration: 0.01)) // For smoothening
    }
    ...
    func  infect(person: SKSpriteNode) {
        person.texture = SKTexture(image: #imageLiteral(resourceName: "ragazzino-04.png"))
    ...
    func triggerGameOver(){
        ...
        player.texture = SKTexture(image: #imageLiteral(resourceName: "Risorsa 9.png"))
        ...
        let gameOverLbl = SKLabelNode(text: "YOU'VE BEEN INFECTED")
